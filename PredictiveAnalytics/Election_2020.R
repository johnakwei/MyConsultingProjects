...
ggplot(data = election2) +
  ...
  geom_text(aes(x = max(Date), y = ElectoralBiden[Date == max(Date)],
                label = round(ElectoralBiden[Date == max(Date)], 0))) +
  geom_line(aes(x = Date, y = ElectoralTrump, group=1,
                color = "ElectoralTrump"), size = 2) +
  geom_text(aes(x = max(Date), y = ElectoralTrump[Date == max(Date)],
                label = round(ElectoralTrump[Date == max(Date)], 0))) +
  ...
  scale_colour_manual(name = " ",
                      values=c("ElectoralBiden"="blue", "ElectoralTrump"="red"),
                      labels=c("Biden", "Trump"))

ggplot(data = election2) +
  geom_line(aes(x = Date, y = PopularBiden, group=1,
                color = "PopularBiden"), size = 2) +
  geom_text(aes(x = max(Date), y = PopularBiden[Date == max(Date)],
                label = round(PopularBiden[Date == max(Date)], 0))) +
  geom_line(aes(x = Date, y = PopularTrump, group=1,
                color = "PopularTrump"), size = 2) +
  geom_text(aes(x = max(Date), y = PopularTrump[Date == max(Date)],
                label = round(PopularTrump[Date == max(Date)], 0))) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) + 
  scale_colour_manual(name = " ",
                      values=c("PopularBiden"="blue", "PopularTrump"="red"),
                      labels=c("Biden", "Trump"))
